...
CGraph::CGraph() : _vertexes(0), _edges(0), _matrix(nullptr), _edgesMatr(nullptr), _arrColors(nullptr), _listVert(nullptr)
{
}

CGraph::CGraph(int vertexes, int edges) : _vertexes(vertexes), _edges(edges), _matrix(nullptr), _edgesMatr(nullptr), _arrColors(nullptr), _listVert(nullptr)
{
}
...
void CGraph::listEdges()
{
	for (int i = 1; i <= _vertexes; i++)
	{
		...
		if (countEd != 0)
		{
			if (countEd == 1)
			{
				for (int p = 0; p < _edges; p++)
				{
					if (_edgesMatr[p][0] == i)
					{
						std::cout << _edgesMatr[p][1];
					}
				}
			}
			else
